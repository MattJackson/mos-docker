#!/bin/bash
# Phase 1 — OEM 10.2.2 + OpenCore EFI on disk + macOS image.
# Adds (vs Phase 0): macOS disk + OpenCore.img + virtio-blk for the macOS
# disk + OpenCoreBoot SATA channel. Still uses OEM (unpatched) QEMU. No
# Apple SMC, no apple-kbd/tablet, no apple-gfx-pci.
# Pass: OpenCore picker rendered through noVNC at http://<host>:6081
# (macOS itself likely won't boot without Apple identity — that's expected).
set -euo pipefail
cd /opt/macos
...
if [ "${CURRENT_SIZE}" -lt 1048576 ]; then
    echo "Empty disk (${CURRENT_SIZE} bytes) — install mode (creating ${DISK_SIZE:-256G})"
...
echo "=== Phase 1 — OEM 10.2.2 + OpenCore (no Apple identity) ==="
...
exec qemu-system-x86_64 \
    ...
    -netdev user,id=net0,hostfwd=tcp::${SSH_PORT:-22221}-:22 \
